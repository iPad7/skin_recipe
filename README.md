...
**배포 URL:** https://skin-recipe.vercel.app
...
자세한 테이블 정의 / JPA Cascade / 인덱스는 [docs/db-schema.md](docs/db-schema.md) 참고.
...
- 표준 성분 DB 연동 및 성분 사전 벡터화 (가능한 경우)
- OCR 프롬프트 고도화 (성분 외 텍스트 필터링)

### v4 — 루틴 고도화
- 루틴을 그래프 구조로 관리 (BHA 데이 / 레티놀 데이 등 노드 분기)
- 주간 루틴 테이블로 요일별 루틴 자동 표시
